...
export default {
  ...
  methods: {
    goDetail (id, title) {
      this.$router.push({name: 'scoredetail', params: {id: id, title: title}})
    },
    handleCurrentChange (value) {
      this.page = value
      this.getActivityListData()
    },
    getScoreListData (value) {
      let param = {
        title: value,
      ...
      this.$api.joinActivityList(param).then(res => {
        if (res.ErrorCode === 0) {
    ...
    searchList () {
      this.getScoreListData(this.inputText)
    }
  },
  mounted () {
